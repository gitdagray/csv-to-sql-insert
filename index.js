...
async function checkFolderExistence(folders) {
	try {
	  ...
	  if (error.code === 'ENOENT') {
		throw new Error(`Folder does not exist: ${error.path}`);
	  } else {
		throw new Error(`Error checking folder: ${error.message}`);
	  }
	}
}
...
async function readCSV(csvFileName = "") {
  try {
	...
	const sql_folderName = './sql';
    const csv_folderName = './csv';

    const folders = [sql_folderName, csv_folderName];

    await checkFolderExistence(folders);
